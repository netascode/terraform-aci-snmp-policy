...
| <a name="input_clients"></a> [clients](#input\_clients) | List of clients. Choices `mgmt_epg`: `inb`, `oob`. | <pre>list(object({<br>    name          = string<br>    mgmt_epg      = optional(string)<br>    mgmt_epg_name = optional(string)<br>    entries = list(object({<br>      ip   = string<br>      name = string<br>    }))<br>  }))</pre> | `[]` | no |
